...
def get_triplet_loss(
    ...
    return torch.maximum(
        cos(anchor, neg) - cos(anchor, pos) + myconfig.TRIPLET_ALPHA, torch.tensor(0.0)
    )
...
def get_triplet_loss_from_batch_output(batch_output, batch_size):
    ...
    )  # batch_output_reshaped.shape=[8,3,128]
    ...
    )  # all the 3rd row will be negative
...
def train_network(speaker_to_utterance, num_steps, saved_model="", pool=None):
    ...
    for step in range(num_steps):
        ...
        batch_input = feature_extraction.get_batched_triplet_input(
            speaker_to_utterance, myconfig.BATCH_SIZE, pool
        )
...
def run_training():
    ...
    speaker_to_utterance = dataset.get_librispeech_speaker_to_utterance(
